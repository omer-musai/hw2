...
namespace mtm
{
    ...
    class SortedList
    {
        ...
        public:
            ...
            template<class C>
            SortedList filter(C condition) const;

            template<class A>
            SortedList apply(A action) const;
    ...
    template<class T>
    template<class C>
    SortedList<T> SortedList<T>::filter(C condition) const
    {
    ...
    template<class T>
    template<class A>
    SortedList<T> SortedList<T>::apply(A action) const
    {
        SortedList<T> new_list = SortedList<T>();
        for (SortedList<T>::const_iterator i = this->begin(); i != this->end(); ++i)
        {
